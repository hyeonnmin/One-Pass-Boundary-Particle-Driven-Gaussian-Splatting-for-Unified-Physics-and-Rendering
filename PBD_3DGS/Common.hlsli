...
struct GSVertexShaderInput
{
    float3 posModel : POSITION;
    float3 scaleModel : SCALE;
    float4 rotationModel : ROTATION;
    float3 shModel : SH;
    float opacityModel : OPACITY;
    float3 normalModel : NORMAL;
    // float3 color : COLOR0; <- 불필요 (쉐이딩)
};


struct GSGeometryShaderInput
{
    float3 posModel : POSITION;
    //float2 texcoord : TEXCOORD;
    float3 scaleModel : SCALE;
...
struct GSPixelShaderInput
{
    float4 posProj : SV_POSITION;
    float3 posWorld : POSITION0;
    float3 posModel : POSITION1;
    float3 normalWorld : NORMAL;
    float2 texcoord : TEXCOORD0; // [-1,1] range
    float3 colorVert : TEXCOORD6;
    float alphaVert : TEXCOORD7;
};
