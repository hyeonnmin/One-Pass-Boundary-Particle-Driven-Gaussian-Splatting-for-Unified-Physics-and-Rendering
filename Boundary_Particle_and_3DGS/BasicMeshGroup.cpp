...
void BasicMeshGroup::Render(ComPtr<ID3D11DeviceContext> &context) {
    ...
    for (const auto &mesh : m_meshes) {
        ...
        context->Draw(mesh->m_meshData.vertices.size(), 0);
    }
...
void BasicMeshGroup::UpdateParticles()
{
    ...
    for (auto &mesh : m_meshes) {
        ...
        //CompactVerticesByScaleZero(meshData);
    }
}
...
void BasicMeshGroup::CompactVerticesByScaleZero(MeshData& meshData)
{
    vector<Vertex> newVertices;
    for (int i = 0; i < meshData.vertices.size(); ++i)
    {
        Vector3 scale = meshData.vertices[i].scale;
        if (!(scale.x == 0.0f && scale.y == 0.0f && scale.z == 0.0f))
            newVertices.push_back(std::move(meshData.vertices[i]));
    }
    meshData.vertices = std::move(newVertices);
}
...
void BasicMeshGroup::PrintParticleInfo() {
        std::cout << "Particle Count: " << m_meshes[0]->m_meshData.vertices.size() << std::endl;
        std::cout << "Particle Distance: " << m_particle_distance << std::endl;
        std::cout << "Particle Size: " << m_gaussian_scaling << std::endl;
