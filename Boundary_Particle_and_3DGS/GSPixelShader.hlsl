...
float4 main(GSPixelShaderInput input) : SV_TARGET {
    ...
    float k = 0.01f;
    float gauss = exp(-k * r2); // 가우시안 weight

    //float baseAlpha = input.alphaVert * gauss;
    ...
    if (usePositionNormal)
        input.normalWorld = normalize(input.posWorld);
    ...
    // 참고: https://www.shadertoy.com/view/lscBW4
     //float3 f = SchlickFresnel(material.fresnelR0, input.posWorld, toEye);
     //specular.xyz *= f;
    ...
    float3 color = diffuse.rgb + specular.rgb;

    if (useLight) {
        int i = 0;

        [unroll] for (i = 0; i < NUM_DIR_LIGHTS; ++i) {
            color += ComputeDirectionalLight(light[i], material,
    ...
    return float4(color, input.alphaVert);
}
